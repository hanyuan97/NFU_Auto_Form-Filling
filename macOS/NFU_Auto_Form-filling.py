...
def poll(URL):
	...
	while True:
		try:
			...
			print u'別亂填啊我懶著處理例外輸入=_='

	return end()

def poll2(tcurl):
	global br, indexUrl
	print u'\n問卷好像有分種類，不保證一定填對，但一定(應該)會幫你填好填滿！！'
	print u'老師(只有好棒棒老師會被推薦教師甄選)：'
	print u'(1)好棒棒！\n(2)好棒\n(3)還行\n(4)好糟\n(5)好糟糟\n(6)老師誰啊隨便填'
...
def main():
	...
	while first or br.geturl() != (indexUrl + funUrl[0]):
		...
		else: print u'帳號密碼或驗證碼錯誤！'
		getauthimg()
		acc = raw_input('帳號：')
		pss = raw_input('密碼：')
		authimg = str(raw_input('驗證碼：')).upper()
